...
P.scripts.testoab = () ->
  ...
  q = @params.q ? '*'
  finds = await @fetch 'https://api.cottagelabs.com/service/oab/availabilities?q=' + q + '&sort=createdAt:desc&from=' + from + '&size=' + size
    ...
    if true #try
      ...
      if JSON.stringify(match) isnt '{}'
        if rec.plugin in ['shareyourpaper', 'instantill']
          ...
          match.config = JSON.parse(rec.config) if rec.config?
        ...
        if rec.permissions?.best_permission?
