...
P.ill_internal = (opts) -> # only worked on POST with optional auth
  if not opts?
    opts = @copy @params
    if opts.ill_internal
      opts.doi = opts.ill_internal
      delete opts.ill_internal
  opts.metadata ?= await @metadata_internal opts
...
P.ill_internal.openurl = (config, meta) ->
  ...
  meta ?= @params.meta ? await @metadata_internal()
...
P.ill_internal.subscription = (config, meta) ->
  if not config and not meta and (@params.sub or @params.subscription) # assume values are being passed directly on GET request
    ...
    if @params.meta
      ...
      meta = await @metadata_internal config.doi
      delete config.doi
    else
      meta = @copy config
      delete config.doi
  ...
  if config.subscription?
    ...
    openurl = await @ill_internal.openurl config, meta
