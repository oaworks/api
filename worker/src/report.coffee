...
P.report.rs.retrieve = (since) ->
  ...
  for await rec from @index._for 'src_crossref_works', q, opts
    ...
    if not refresh and not plus and exists = await @report.works rec.DOI
      ...
      res.existing += 1
    else if not refresh and not empty and local = await @report.rs 'identity.keyword:"' + rsid + '"' # rec.DOI
...
P.report.rs.changes = (since) ->
  ...
  if not since
    last = await @report.rs '*', sort: 'createdAt.keyword:desc', size: 1
  ...
  for doi in dois = (await fs.readFile @S.static.folder + '/rscheck.csv', 'utf8').split '\n'
    ...
    if doi
      ...
      if id
        ...
        res.checked += 1
        exists = false
        if exists = await @report.rs 'identity.keyword:"' + id + '"'
