...
P.metadata_grace = (doi) ->
  res = await @find_grace doi # may not be a DOI, but most likely thing
  return res?.metadata
P.metadata_grace._log = false

P.find_grace = (options, metadata={}, content) ->
  res = {}

  _metadata = (input) =>
    ct = await @citation_grace input
    for k of ct
      if k in ['url', 'paywall']
        res[k] ?= ct[k]
      else
  ...
  options.find_grace = options.metadata if options.metadata
  if options.metadata_grace
    options.find_grace = options.metadata_grace
    delete options.metadata_grace
  if options.find_grace
    if options.find_grace.startsWith('10.') and options.find_grace.includes '/'
      options.doi = options.find_grace
    else
      options.url = options.find_grace
    delete options.find_grace
  ...
  _ill = () =>
    if (metadata.doi or (metadata.title and metadata.title.length > 8 and metadata.title.split(' ').length > 1)) and (options.from or options.config?) and (options.plugin is 'instantill' or options.ill is true)
      try res.ill ?= subscription: await @ill_grace.subscription (options.config ? options.from), metadata
    return true
  _permissions = () =>
...
P.citation_grace = (citation) ->  
  try citation ?= @params.citation_grace ? @params
