...
P.find_diffs = ->
  ...
  res = count: [], ignoring: ignoring, dois: [
    ...
    '10.1111/j.1600-051x.1988.tb01596.x',
    '10.1186/s12888-019-2120-9',
    '10.1021/acsanm.8b00798',
    '10.1001/archinte.1963.03860030092007',
    '10.1001/jamainternmed.2019.6770',
    '10.1090/s0002-9939-2014-12202-7',
    '10.1045.joes-fake-doi',
    '10.3726/978-3-653-04227-6',
    '10.1016/b978-1-78242-121-4.00003-4',
    '10.1036/1097-8542.198450',
    '10.1063/1.4905616',
    '10.1080/03071843809419849',
    '10.1134/s1068162014030066',
    '10.15650/hebruniocollannu.89.2018.0221',
    '10.1111/j.1474-919x.1995.tb08455.x',
    '10.2164/jandrol.111.014894',
    '10.1111/j.1600-051x.1988.tb01596.x',
    '10.1103/physreve.83.066702',
    '10.1016/j.foot.2017.04.003',
    '10.1134/S1019331619060042'
  ]
  res.find = {}
  for doi in res.dois
    res.find[doi] = {}
    o = await @find_old doi
    try # sort old author affiliations, simple way to compare
      for au in o.metadata.author
        try au.affiliation = au.affiliation.sort (a,b) -> a.name.localeCompare(b.name)
    # and sort author strings for compare too
    try o.metadata.author = o.metadata.author.sort (a,b) -> a.name.localeCompare(b.name)
    n = await @find doi
    try n.metadata.author = n.metadata.author.sort (a,b) -> a.name.localeCompare(b.name)
    if (o.url ? '').toLowerCase() isnt (n.url ? '').toLowerCase()
      res.find[doi].url_old = o.url ? ''
      res.find[doi].url_new = n.url ? ''
    for k of o.metadata
      o.metadata[k] = JSON.stringify(o.metadata[k]) if typeof o.metadata[k] is 'object'
      n.metadata[k] = JSON.stringify(n.metadata[k]) if typeof n.metadata[k] is 'object'
      if k not in res.ignoring and (o.metadata[k] ? '').toLowerCase() isnt (n.metadata[k] ? '').toLowerCase()
        res.find[doi][k + '_old'] = o.metadata[k] ? ''
        res.find[doi][k + '_new'] = n.metadata[k] ? ''
        res.find[doi][k + '_lev'] = (await @levenshtein res.find[doi][k + '_old'], res.find[doi][k + '_new']).distance if res.find[doi][k + '_old'] and res.find[doi][k + '_new']
        res.count.push(doi) if doi not in res.count
    for c of n.metadata
      if c not in res.ignoring and n.metadata[c] and not o.metadata[c]?
        res.find[doi][c + '_old'] = ''
        res.find[doi][c + '_new'] = if typeof n.metadata[c] is 'object' then JSON.stringify(n.metadata[c]) else (n.metadata[c] ? '')
        res.count.push(doi) if doi not in res.count
  res.count = res.count.length + '/' + res.dois.length
  return res

P.find_old = (options, metadata={}, content) ->
  res = {}

  _metadata = (input) =>
    ct = await @citation_old input
    for k of ct
      if k in ['url', 'paywall']
        res[k] ?= ct[k]
      else
  ...
  options.find_old = options.metadata if options.metadata
  if options.find_old
    if options.find_old.startsWith('10.') and options.find_old.includes '/'
...
P.citation_old = (citation) ->
  res = {}
  
  try citation ?= @params.citation_old ? @params
