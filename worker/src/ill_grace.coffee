...
P.ill_grace = (opts) -> # only worked on POST with optional auth
  if not opts?
    opts = @copy @params
    if opts.ill_grace
      opts.doi = opts.ill_grace
      delete opts.ill_grace
  opts.metadata ?= await @metadata_grace opts
...
P.ill_grace.openurl = (config, meta) ->
  ...
  meta ?= @params.meta ? await @metadata_grace()
...
P.ill_grace.subscription = (config, meta) ->
  if not config and not meta and (@params.sub or @params.subscription) # assume values are being passed directly on GET request
    ...
    if @params.meta
      ...
      meta = await @metadata_grace config.doi
      delete config.doi
    else
      meta = @copy config
      delete config.doi
  ...
  if config.subscription?
    ...
    openurl = await @ill_grace.openurl config, meta
