...
try S.src.sherpa = JSON.parse SECRETS_SHERPA
...
P.src.sherpa.opendoar.import = () ->
  await @src.sherpa.opendoar ''
  recs = []
  counter = 0
  offset = 0
  while res = await @fetch 'https://v2.sherpa.ac.uk/cgi/retrieve?api-key=' + @S.src.sherpa.apikey + '&item-type=repository&format=Json&offset=' + offset
