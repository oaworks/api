        ...
        if (a[k]._schedule && !_schedule[nd] && this.S.bg === true && this.S.cron !== false) {
          ...
          sfn = (fnm) => {
            return async() => {
              ...
              if (this.S.dev !== true && !this.S.async && !this.S.async_loop && !this.S.async_schedule && ((ref18 = process.env.pm_id) !== 1 && ref18 !== '1')) {
              ...
              } else if (typeof aru === 'string' && !process.env.name.endsWith(((ref19 = fno._runner) != null ? ref19 : fnm).replace(/\./g, '_'))) {
...
// tidy any params provided within the URL
  ...
  if (((ref14 = typeof fn) === 'object' || ref14 === 'function') && fn._bg && typeof this.S.bg === 'string' && this.S.bg.startsWith('http')) {
    throw new Error();
  } else if (((ref15 = typeof fn) === 'object' || ref15 === 'function') && fn._async && typeof this.S.async === 'string' && (typeof ((ref16 = this.S.async_runner) != null ? ref16[(ref17 = fn._runner) != null ? ref17 : this.fn] : void 0) !== 'string' || !process.env.name.endsWith(((ref18 = fn._runner) != null ? ref18 : this.fn).replace(/\./g, '_')))) {
...
P.report.queue = async function(doi) {
  var batch, d;
  if (doi == null) {
    doi = this.params.queue;
  }
  ...
  if (doi == null) {
    if (!_do_batch.length) {
      q = (await this.report.queued('*', {
        size: 100,
        sort: {
          createdAt: 'asc' // if running more than one queue processor, this will need a better approach
        }
      }));
      ...
      for (l = 0, len1 = ref2.length; l < len1; l++) {
        ...
        if ((!orgname || org.name === orgname) && (!sheetname || s.name === sheetname)) {
            ...
            try {
              ...
              if (Array.isArray(check) && check.length && check[0][0].toLowerCase() === 'last updated' && (last = check[0][1])) {
                ...
                if (typeof last === 'number' && last > 1672531200000) { // start of 2023, just in case of a bad date
                  ...
                  if (((ref4 = latest.hits) != null ? ref4.hits : void 0) != null) {
          ...
          if (update !== true) {
              ...
              for (n = 0, len3 = rows.length; n < len3; n++) {
                ...
                for (hp in headers) {
                  ...
                  if (h === 'doi' || h === 'DOI') {
                    ...
                    hpv = '';
                    if (h === 'apc_cost' || h === 'wellcome.apc_paid_actual_currency_excluding_vat' || h === 'wellcome.apc_paid_gbp_inc_vat_if_charged' || h === 'wellcome.additional_publication_fees_gbp' || h === 'wellcome.amount_of_apc_charged_to_coaf_grant_inc_vat_if_charged_in_gbp' || h === 'wellcome.amount_of_apc_charged_to_rcuk_oa_fund_inc_vat_if_charged_in_gbp') {
                      try {
                ...
                if (rr.DOI = (await this.report.cleandoi(rr.DOI))) {
                  ...
                  if (ref10 = rr.DOI, indexOf.call(dois, ref10) < 0) {
                    kc = false;
                    if (!clear) {
                      present = (await this.report.works('supplements.osdid.keyword:"' + rr.osdid + '"', 1));
                      if (((present != null ? (ref11 = present.hits) != null ? ref11.hits : void 0 : void 0) != null) && present.hits.hits.length) {
                        present = present.hits.hits[0]._source;
                      }
                      try {
                        ref12 = present.supplements;
                      ...
                      if (present != null) {
                        ...
                        delete kc.updated;
                        for (k in present) {
                          if ((k !== 'updated') && JSON.stringify((ref13 = rr[k]) != null ? ref13 : '').toLowerCase() !== JSON.stringify(present[k]).toLowerCase()) { // JSON string match on object isn't guaranteed but probably likely enough for the number of times we'll need it
                            break;
                          }
                          delete kc[k];
                        }
            ...
            ref14 = this.index._for('paradigm_' + (this.S.dev ? 'b_' : '') + 'report_orgs_supplements', 'org.keyword:"' + org.name + '" AND sheets.keyword:"' + s.name + '"', {
              scroll: '30m',
              include: ['osdid']
            });
            for await (sup of ref14) {
              if (ref15 = sup.osdid, indexOf.call(osdids, ref15) < 0) {
                await this.report.orgs.supplements(sup.osdid, '');
                deletes.push(sup.osdid);
                if (ref16 = sup.DOI, indexOf.call(dois, ref16) < 0) { // need to rerun for ones where something has been deleted too so that deleted supplements get removed from the work
                  dois.push(sup.DOI);
                }
              }
            }
  ...
  if (!clear && !orgname && !sheetname) {
    ...
    for (r = 0, len5 = ref17.length; r < len5; r++) {
      ...
      if (indexOf.call(sheetnames, aps) < 0) {
        ...
        ref19 = this.index._for('paradigm_' + (this.S.dev ? 'b_' : '') + 'report_works', 'supplements.sheets.keyword:"' + aps + '"', {
          scroll: '30m',
          include: ['DOI']
        });
        for await (osw of ref19) {
          if (osw.DOI && (ref20 = osw.DOI, indexOf.call(dois, ref20) < 0)) {
            dois.push(osw.DOI);
          }
        }
      }
...
P.report.works.process = async function(cr, openalex, refresh, everything, replaced) {
  var _rsup, a, ad, ass, assl, best_initial, best_name, best_score, brd, c, email, epmc, exists, f, i, i1, j, k, l, lc, len, len1, len10, len11, len12, len2, len3, len4, len5, len6, len7, len8, len9, lic, loc, lvs, m, n, newer, oadoi, ok, ox, p, permissions, pfn, pp, prds, pt, pubmed, r, ran, rec, ref, ref1, ref10, ref11, ref12, ref13, ref14, ref15, ref16, ref17, ref18, ref19, ref2, ref20, ref21, ref22, ref23, ref24, ref25, ref26, ref27, ref28, ref29, ref3, ref30, ref31, ref32, ref33, ref34, ref35, ref36, ref37, ref38, ref39, ref4, ref40, ref41, ref42, ref43, ref44, ref45, ref46, ref47, ref48, ref49, ref5, ref50, ref51, ref52, ref53, ref54, ref55, ref56, ref6, ref7, ref8, ref9, ren, rn, rr, score, sd, started, sup, u, ud, ude, v, w, x, xref, y, z;
  started = (await this.epoch());
  if (cr == null) {
  ...
  if (!(exists != null ? exists.oadoi : void 0) && rec.DOI) {
    ...
    for (x = 0, len9 = ref33.length; x < len9; x++) {
      loc = ref33[x];
    ...
    if (everything) {
      if (!rec.PMCID || !rec.pubtype) { //or not rec.submitted_date or not rec.accepted_date # only thing restricted to orgs supplements for now is remote epmc lookup and epmc licence calculation below
        if (epmc = (await this.src.epmc.doi(rec.DOI))) {
          ...
          for (i1 = 0, len12 = ref54.length; i1 < len12; i1++) {
            pt = ref54[i1];
...
P.report.works.load = async function(timestamp, org, dois, year, refresh, supplements, everything, info) {
  ...
  if (Array.isArray(dois)) {
    ...
    _crossref = async(cq) => {
      var cr, l, len1, processed, ref7, ref8, results;
      processed = cc.length;
      if (cq == null) {
    ...
    _openalex = async(oq) => {
      var l, len1, o, ol, oodoi, processed, ref7, ref8, ref9, results;
      processed = oo.length;
      if (oq == null) {
