...
P.search = (opts) ->
  ...
  opts.fuzzify = (str) ->
    if opts.fuzzy and str.indexOf('*') is -1 and str.indexOf('~') is -1 and str.indexOf(':') is -1 and str.indexOf('"') is -1  and str.indexOf('AND') is -1 and str.indexOf('OR') is -1
  ...
  opts.result = (rec) -> # a function that draws a given result on the page (can be false to not draw results)
    ...
    for k of (if opts.table then opts.table else rec)
      kk = if typeof opts.table is 'object' and opts.table[k] then opts.table[k] else k
      P.append('.PSearchHeaders', '<th>' + kk + '</th>') if opts.table and headers.indexOf(kk) is -1
      ...
      re += (if typeof rec[k] is 'object' then JSON.stringify(rec[k]) else rec[k]) if rec[k] #opts.link {text:rec[k][i],val:k+':'+rec[k][i]+''}
  ...
  if not opts.template and opts.template isnt false
    ...
    opts.template += if opts.table then '<table class="striped"><thead' + (if opts.sticky then ' class="sticky"' else '') + '><tr class="PSearchHeaders"></tr></thead><tbody class="PSearchResults"></tbody></table>' else '<div class="PSearchResults"></div>'
